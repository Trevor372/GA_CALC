...
tau_b(double V, double taub)
{
    //taub(v)=taub0 + (taub-taub0)/(1+exp(-(v-tb)/sb))
    return 10 + (taub-10)/(1.0+exp(-(V-(-80))/10));
}
*/

extern "C" Plugin::Object *createRTXIPlugin(void)
{
	return new GA_Calc_SGA();
}
...
static DefaultGUIModel::variable_t vars[] = 
{
	...
	{"Specific_GA", "SGA determined from data", DefaultGUIModel::PARAMETER | DefaultGUIModel::DOUBLE},
	{"IA_Scale", "Scalar factor for output current", DefaultGUIModel::PARAMETER | DefaultGUIModel::DOUBLE},
	...
	{"On/Off", "1 = on, 0 = off", DefaultGUIModel::PARAMETER | DefaultGUIModel::UINTEGER, },
    	...
    	{"taub", "",DefaultGUIModel::PARAMETER | DefaultGUIModel::DOUBLE, },
...
#define da (dydt[0])
#define db (dydt[1])
#define GACalc (Specific_GA*cm*IA_Scale*a*a*a*b)

GA_Calc_SGA::GA_Calc_SGA(void) : DefaultGUIModel("GA_Calc_SGA", ::vars, ::num_vars)
{
...
GA_Calc_SGA::~GA_Calc_SGA(void){}

void GA_Calc_SGA::execute(void)
{
...
void GA_Calc_SGA::update(DefaultGUIModel::update_flags_t flag){
	switch(flag){
		case INIT:
			setParameter("V0 (mV)", QString::number(V0)); // initialized in mV, display in mV
			setParameter("IA_Scale", QString::number(IA_Scale)); // scalar factor for current
...
void GA_Calc_SGA::initParameters() {
	V0 = -55.038; // mV
	IA_Scale = 1;
	Specific_GA = 4.38;
	EA = -100;
...
void GA_Calc_SGA::solve(double dt, double *y, double V){
	double dydt[2];
	derivs(y, dydt,V);
...
void GA_Calc_SGA::derivs(double *y, double *dydt, double V){
	GA = GACalc;	
	IA = -GACalc*(V - EA);
	IACell = IA * 2e-9;
